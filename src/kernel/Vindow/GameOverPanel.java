...
public class GameOverPanel extends JPanel {
    ...
    private void initUI() {
        ...
        if (testConnect()) {
            ...
            model.setColumnIdentifiers(new String[]{"Имя", "Яблок", "Время"});
            ...
            for (HashMap<String, String> record : data) {
                String[] rowData = new String[]{record.get("name"), record.get("apple"), record.get("time")};
                model.addRow(rowData);
            }
            databaseController.connect.closeConnection();
        } else {
            // Данные для таблицы при отсутствии подключения
            String[] columnNames = {"Имя", "Яблок", "Время"};
            Object[][] data = {
